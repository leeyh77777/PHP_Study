 ...
 */
 public function join($data) {
	 /**
	  * 1.필수 항목 체크
	  * 2.아이디(자리수, 알파벳&숫자 만 허용, 정규표현식)
	  * 3.아이디 중복여부
	  * 4.비밀번호 (자리수 8자리 이상, 반드시 알파벳, 숫자, 특수문자 포함) + 비밀번호 확인
	  * 4.휴대전화번호 선택 - 휴대전화번호 입력된 경우 : 형식체크 , 입력데이터의 통일 (숫자만 입력)
	  *
	  */
	  // 필수항목체크
	 $required = [
	 ...
	 $pattern = "/[^0-9a-z]/i"; // 알파벳, 숫자 이외의 문자
	...
	$sql = "INSERT INTO php_member (memId, memPw, memNm, cellPhone)
				VALUE (:memId, :memPw, :memNm, :cellPhone)";
	$stmt = $this->db->prepare($sql);
	$stmt->bindValue(":memId", $memId);
...
public function update($data) {
	...
	// 비밀번호 체크 (비밀번호 변경 시도시)
	$hash = "";
	if (isset($data['memPw']) && trim($data['memPwRe'])) {
	...
	public function checkPassword($memPw, $memPwRe) {
		...
		if (!preg_match("/[a-z]/i", $memPw) || !preg_match("/[0-9]/", $memPw) || !preg_match("/[~!@#$%^&*()]/", $memPw)) {
			throw new Exception("비밀본흐는 알파벳, 숫자, 특수문자를 반드시 1개이상 포함하세요.");
		}
		
		// 비밀번호 확인 일치여부
		if ($memPw != $memPwRe) {
			throw new Exception("비밀번호확인이 일치 하지 않습니다.");
		}
	}
